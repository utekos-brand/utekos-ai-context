...
Project subagents override user subagents with the same name (see [precedence rules](https://docs.langchain.com/oss/javascript/deepagents/code/data-locations#subagents)).

---

## Frontmatter Requirements

The frontmatter requires `name` and `description` (same as the [`SubAgent`](https://docs.langchain.com/oss/javascript/deepagents/subagents#subagent-dictionary-based) [ dictionary spec](https://docs.langchain.com/oss/javascript/deepagents/subagents#subagent-dictionary-based)).

The markdown body becomes the subagent’s `system_prompt`.

### Optional Fields

* `model`: Overrides the main agent’s model for this subagent
...
Other `SubAgent` fields are currently not configurable via `AGENTS.md` frontmatter:
...
Custom subagents defined this way inherit the main agent’s tools.
...
This overrides the built-in general-purpose subagent, routing all delegated tasks to a cheaper model.

See [Override the general-purpose subagent](https://docs.langchain.com/oss/javascript/deepagents/subagents#override-the-general-purpose-subagent) for more.
