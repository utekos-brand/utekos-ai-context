...
Alternatively, you can set the `Vercel-Cache-Tag` response header with a comma-separated list of tags instead of using `addCacheTag()`. See [cache tags](/docs/cdn-cache/purge#cache-tags) for more details.
...
When using `experimental_upgradeWebSocket()` in a Next.js app with Cache Components enabled, call [`connection()`](https://nextjs.org/docs/app/api-reference/functions/connection) before `experimental_upgradeWebSocket()`. This opts the route handler out of static prerendering, so the WebSocket upgrade runs only at request time.
